...
Pacf(count_ma, main='')
```
We can start with the order of d = 1 and re-evaluate whether further differencing is needed. The augmented Dickey-Fuller test on differenced data rejects the null hypotheses of non-stationarity. However, since the augmented Dickey-Fuller test show `p-value` greater than 0.05, so we cannot reject null hypothesis. 
```{r}
...
adf.test(count_d1, alternative = "stationary")
```
Plotting the differenced series, we see an oscillating pattern around 0 with no visible strong trend. This suggests that differencing of order 2 terms is sufficient and should be included in the model.
...
- From the PACF plot there is a spike at 1 so we will choose d=2 
- Or We could leverage `auto.arima` to automatically generate a set of optimal (p, d, q)
```{r}
auto.arima(deseasonal_cnt, seasonal=FALSE,d = 2)
...
```{r}
fit<-auto.arima(deseasonal_cnt, seasonal=FALSE,d = 2)
tsdisplay(residuals(fit), main='(2,2,4) Model Residuals')
```
